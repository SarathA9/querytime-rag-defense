...
\centering
\vspace*{2.5cm}
{\large AI Security and Privacy --- Project Work (Exam)\par}
\vspace{0.4cm}
{\small Prof.\ Dr.\ Patrick Levi \quad$\cdot$\quad OTH Amberg-Weiden \quad$\cdot$\quad MSc Artificial Intelligence\par}
\vspace{2.2cm}
{\LARGE\bfseries Retrieval-Time Backdoor Detection in RAG-based LLMs via Rank-Faithfulness Consistency Checking\par}
\vspace{1.4cm}
{\large Sarath Adukkadukkam\par}
{\small s.adukkadukkam@oth-aw.de\par}
\vspace{2.2cm}
{\small\itshape A query-time, retrieval-layer detector evaluated against a dual-surface
(fine-tuning + corpus) backdoor threat on LLaMA-3-8B / Natural Questions.\par}
\vfill
{\small \today \quad$\cdot$\quad Body text: \textasciitilde1{,}960 words\par}
\end{titlepage}
